...
@Component({
  selector: 'app-vlasnikregister',
  templateUrl: './vlasnikregister.component.html',
  styleUrls: ['./vlasnikregister.component.css']
})
export class VlasnikregisterComponent {
      ...
      if (this.korisnicko_ime != '' && this.lozinka != '' && this.ime != '' && this.prezime != '' && this.adresa != '' &&
        this.telefon != '' && this.email != '' && this.broj_kartice != ''
          ...
          this.korisniciServis.registerKorisnik(this.korisnicko_ime, this.lozinka, this.ime, this.prezime, this.pol, this.adresa,
            this.telefon, this.email, this.profilna_slika, this.broj_kartice, 'vlasnik', 'u obradi', ''
          ).subscribe(
            mess => {
              if (mess == 'ok') {
                this.messageRegisterError = ''
                this.messageRegisterSucces = 'Uspešno ste izvršili registraciju. Vaš zahtev je sada u obradi i biće obrađen u najkraćem mogućem roku.'
                this.ocistiSve();
              } else {
